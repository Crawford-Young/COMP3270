...
class ProgrammingAssignment1 {
	...
	//Function to compute the median of three numbers. Used to benchmark run times for sorting algorithms.
	...
	private static int partition(int data[], int begin, int end) {
	    ...
	    data[end] = swapHelper(data[i+1], data[i+1]=data[end]);
	...
	static int[] insertionSort(int[] data, int n){
		for(int i = 0; i<n; i++) {
			...
			while(j>=1 && data[j]<data[j-1]) {
				data[j] = swapHelper(data[j-1], data[j-1]=data[j]);
				j--;
			}
		}
		return data;
	...
	public static void main(String[] args) {
		...
		System.out.println("Array type: " + arrayType + " Array Length: " + n +
				" Algo: " + algo);
		int[] data = new int[n];
		...
		if(!success){
			...
			System.out.println("Median Execution time: " +
					totalTime[medianof3(totalTime[0], totalTime[1], totalTime[2]) - 1] + " ns");
		}
	}
}
//"C:\Users\young\eclipse-workspace\COMP3270A1\src\ProgrammingAssignment1.java"